...
TRANSLATIONS = {
  'ar': {
    'app_title':'نظام محاسبة المكتبة','default_library':'مكتبتي',
  ...
  'en': {
    'app_title':'Bookstore Accounting','default_library':'My Library',
  ...
  'tr': {
    'app_title':'Kitapçı Muhasebesi','default_library':'Kütüphanem',
...
def invoice_new():
    ...
    if request.method == 'POST':
        ...
        for i in range(len(ids)):
            ...
            b = cur.fetchone()
            if not b: continue
            price = float(prices[i]); qty = int(qtys[i])
...
def invoice_view(inv_id):
    ...
    inv = cur.fetchone(); con.close()
    if not inv: return "Not found", 404
    inv = dict(inv); inv['items'] = json.loads(inv['items_json'])
...
# ✅ نضمن إنشاء الجداول حتى عندما يشغل Render التطبيق عبر gunicorn
init_db()
